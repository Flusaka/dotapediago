...
func (client *Client) GetMatches() ([]*Match, error) {
	...
	matches := client.parseMatches(matchesGroups.Eq(0))
	return matches, nil
}

func (client *Client) parseMatches(table *goquery.Selection) []*Match {
	var matches []*Match
	table.Find(".infobox_matches_content").Each(func(i int, selection *goquery.Selection) {
		match := client.parseMatch(selection)
		matches = append(matches, match)
	})
	return matches
}

func (client *Client) parseMatch(row *goquery.Selection) *Match {
